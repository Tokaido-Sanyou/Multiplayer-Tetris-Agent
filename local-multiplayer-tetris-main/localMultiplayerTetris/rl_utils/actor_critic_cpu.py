...
class ActorCritic(nn.Module):
    """
    Actor-Critic network for Tetris (CPU-only version)
    """
...
class ActorCriticAgentCPU:
    """
    Actor-Critic agent with epsilon-greedy exploration (CPU-only version)
    """
    def __init__(self, state_dim, action_dim, actor_lr=1e-4, critic_lr=1e-3, gamma=0.99, 
        ...
        """
        Initialize Actor-Critic agent (CPU-only)
        """
    ...
    def select_action(self, state):
        ...
        Args:
            state: Numpy array of shape (202,) containing preprocessed state
        Returns:
