...
class DQNTrainer:
    ...
    def __init__(self, 
        ...
        self.agent = DQNAgent(
            action_space_size=self.model_config['output_size'],
    ...
    def select_action(self, observation: Any, training: bool = True) -> int:
        ...
        return self.agent.select_action(observation, training)
    ...
    def convert_action_to_tuple(self, action_idx: int) -> List[int]:
        ...
        return list(self.agent.convert_action_to_tuple(action_idx))
    ...
    def train_step(self):
        ...
        # Use agent's batch update method
        batch_states = [e.state for e in experiences]
        batch_actions = [e.action for e in experiences]
        batch_rewards = [e.reward for e in experiences]
        batch_next_states = [e.next_state for e in experiences]
        batch_dones = [e.done for e in experiences]
        ...
        if self.config['prioritized_replay'] and 'td_errors' in update_metrics:
            self.memory.update_priorities(indices, update_metrics['td_errors'].tolist())
        
        return update_metrics['loss']
    ...
    def save_checkpoint(self, episode: int, metrics: Dict[str, float]):
        ...
        training_path = os.path.join(self.checkpoint_dir, f"training_state_ep{episode}.pt")
        torch.save(training_state, training_path)
        
        # Save best model
        if 'mean_reward' in metrics:
            best_path = os.path.join(self.checkpoint_dir, "best_agent.pt")
            if not os.path.exists(best_path):
                self.agent.save_checkpoint(best_path)
                self.logger.info(f"💾 First model saved as best: {metrics['mean_reward']:.3f}")
            else:
                best_training_state = os.path.join(self.checkpoint_dir, "best_training_state.pt")
                if os.path.exists(best_training_state):
                    best_state = torch.load(best_training_state)
                    if metrics['mean_reward'] > best_state['metrics'].get('mean_reward', -float('inf')):
                        self.agent.save_checkpoint(best_path)
                        torch.save(training_state, best_training_state)
    ...
    def evaluate(self, num_episodes: int = 10) -> Dict[str, float]:
        ...
        for ep in range(num_episodes):
            ...
            while not done and steps < self.config['max_steps_per_episode']:
                action_idx = self.select_action(obs, training=False)
        ...
        try:
            while self.episode < self.config['max_episodes']:
                ...
                while not done and episode_steps < self.config['max_steps_per_episode']:
                    ...
                    action_idx = self.select_action(obs, training=True)
                    action = self.convert_action_to_tuple(action_idx)
                ...
                self.metrics_tracker.add_metric('epsilon', self.agent.epsilon)
