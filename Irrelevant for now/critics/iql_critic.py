...
class IQLCritic(BaseCritic):
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE START HERE ###
        value_loss = self.expectile_loss(self.q_net_target(ob_no, ac_na) - self.v_net(ob_no)).mean()
        ### YOUR CODE END HERE ###
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ### YOUR CODE START HERE ###
        loss = self.mse_loss(reward_n + (1.0 - terminal_n) * self.gamma * self.v_net.forward(next_ob_no), self.q_net.forward(ob_no, ac_na))
        ### YOUR CODE END HERE ###
