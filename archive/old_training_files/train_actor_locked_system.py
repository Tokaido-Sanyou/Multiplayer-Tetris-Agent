...
class ActorLockedTrainer:
    ...
    def __init__(self,
        ...
        log_dir = os.path.join("logs", "actor_locked_system")
    ...
    def train(self, 
        ...
        for episode in range(num_episodes):
            ...
            # TensorBoard logging
            self.writer.add_scalar('Episode/Reward', episode_result['reward'], actual_episode)
            self.writer.add_scalar('Episode/Pieces', episode_result['pieces'], actual_episode)
            self.writer.add_scalar('Episode/Lines', episode_result['lines'], actual_episode)
            self.writer.add_scalar('Loss/Locked', episode_result['locked_loss'], actual_episode)
            self.writer.add_scalar('Loss/Actor', episode_result['actor_loss'], actual_episode)
            self.writer.add_scalar('Actor/SuccessRate', episode_result['actor_success_rate'], actual_episode)
            if 'locked_epsilon' in episode_result:
                self.writer.add_scalar('Hyperparams/Epsilon', episode_result['locked_epsilon'], actual_episode)
        ...
        # After training loop, close TensorBoard writer
        self.writer.close()
    ...
    def _run_episode(self, episode_num: int, show_visualization: bool = False) -> Dict[str, Any]:
        ...
        return {
            ...
            'locked_epsilon': self.agent.locked_model.epsilon
        }
