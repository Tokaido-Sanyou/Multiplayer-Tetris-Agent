...
class TetrisEnv(gym.Env):
    ...
    def switch_mode(self, num_agents: int = None, step_mode: str = None):
        ...
        if num_agents is not None:
            ...
            single_action_space = spaces.Box(low=0, high=1, shape=(8,), dtype=np.int8)
            single_obs_space = self.observation_space if self.num_agents == 1 else list(self.observation_space.spaces.values())[0]
    ...
    def _get_single_agent_observation(self, agent_idx: int):
        """Get observation for a single agent with enhanced features"""
        if agent_idx >= len(self.players):
            # Return empty observation if agent doesn't exist
            return {
                'piece_grids': np.zeros((7, 20, 10), dtype=np.int8),
                'current_piece_grid': np.zeros((20, 10), dtype=np.int8),
                'empty_grid': np.zeros((20, 10), dtype=np.int8),
                'next_piece': np.zeros(7, dtype=np.int8),
                'hold_piece': np.zeros(7, dtype=np.int8),
                'current_rotation': np.int8(0),
                'current_x': np.int8(0),
                'current_y': np.int8(0),
        ...
        # Get grids for all possible piece placements
        piece_grids = np.zeros((7, 20, 10), dtype=np.int8)
        for i, shape in enumerate(shapes):
            piece = Piece(5, 0, shape)
            formatted_piece = convert_shape_format(piece)
            for j, pos in enumerate(formatted_piece):
                if 0 <= pos[1] < 20 and 0 <= pos[0] < 10:
                    piece_grids[i][pos[1]][pos[0]] = 1
        
        # Current piece grid
        current_piece_grid = np.zeros((20, 10), dtype=np.int8)
        if player.current_piece:
            formatted_piece = convert_shape_format(player.current_piece)
            for pos in formatted_piece:
                if 0 <= pos[1] < 20 and 0 <= pos[0] < 10:
                    current_piece_grid[pos[1]][pos[0]] = 1
        
        # Empty grid (inverse of locked positions)
        grid = create_grid(player.locked_positions)
        empty_grid = np.zeros((20, 10), dtype=np.int8)
        for i in range(20):
            for j in range(10):
                empty_grid[i][j] = 1 if grid[i][j] == (0, 0, 0) else 0
        ...
        if player.next_pieces and len(player.next_pieces) > 0:
            ...
            if next_shape in shapes:
                next_piece[shapes.index(next_shape)] = 1
        ...
        if player.hold_piece:
            ...
            if hold_shape in shapes:
                hold_piece[shapes.index(hold_shape)] = 1
        
        # Current piece properties
        current_rotation = np.int8(player.current_piece.rotation if player.current_piece else 0)
        current_x = np.int8(player.current_piece.x if player.current_piece else 0)
        current_y = np.int8(player.current_piece.y if player.current_piece else 0)
        ...
        if self.num_agents > 1 and len(self.players) > 1:
            ...
            if opponent_idx < len(self.players):
                opponent_grid_raw = create_grid(self.players[opponent_idx].locked_positions)
                for i in range(20):
                    for j in range(10):
                        opponent_grid[i][j] = 0 if opponent_grid_raw[i][j] == (0, 0, 0) else 1
    ...
    def get_state_features(self, agent_idx: int = 0):
        """Extract engineered features from current state"""
        if agent_idx >= len(self.players):
            return {}
